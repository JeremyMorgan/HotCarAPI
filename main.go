...
import (
	...
	"github.com/jeremymorgan/HotCarAPI/models"
...
func main() {
	...
	http.HandleFunc("/Days", func(w http.ResponseWriter, r *http.Request) {

		var daysofweek []models.DayofWeek
		...
		rows, err := db.Query("select DISTINCT(substr(TimeStamp, 0, 11)) from Reading")
		if err != nil {
			log.Fatal(err)
		}
		...
		for rows.Next() {
			...
			err := rows.Scan(&dayofWeek.DayofWeek)
			if err != nil {
				log.Fatal(err)
			}
		...
		if len(daysofweek) > 0 {
			...
			json.NewEncoder(w).Encode(daysofweek)
		} else {
